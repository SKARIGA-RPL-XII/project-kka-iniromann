...
<body class="bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-2">{{ $pengajuan->nomor_pengajuan }}</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm border">
            <div class="p-6 border-b border-gray-100">
                <h3 class="text-lg font-semibold text-gray-900">Form Edit Pengajuan</h3>
            </div>
            
            <form method="POST" action="{{ route('admin.pengajuan.update', $pengajuan->id) }}" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mt-6" id="file-surat-section" style="{{ old('status', $pengajuan->status) == 'Selesai' ? 'display: block;' : 'display: none;' }}">
                    ...
                    @if($pengajuan->file_surat)
                        <p class="text-xs text-blue-600 mt-1">File saat ini: {{ basename($pengajuan->file_surat) }}</p>
                    @endif
                    @error('file_surat')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
